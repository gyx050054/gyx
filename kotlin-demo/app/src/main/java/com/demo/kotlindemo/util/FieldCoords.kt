/**
 * 【文件职责】
 * 田块坐标工具：在田块（asset）本身没有经纬度时，按田块名提供一套默认中心坐标，
 * 并计算田块内某台设备的示意点位，让「按田块天气」与「田块地图设备点位」即使在
 * 无真实坐标时也能展示真实分布。
 *
 * 【数据流】
 * 输入：田块名（centerFor）、田块中心坐标 + 设备在田块内的编号（devicePoint）。
 * 逻辑：centerFor 按田块名子串匹配默认坐标表，匹配不到用 FALLBACK 兜底；
 *       devicePoint 用田块中心 + 按 index 划分的 3x3 网格偏移，避免设备全部叠在同一点。
 * 输出：Pair<Double, Double>（lat to lon），供地图层放置田块/设备标记。
 */
// 声明包名：工具层
