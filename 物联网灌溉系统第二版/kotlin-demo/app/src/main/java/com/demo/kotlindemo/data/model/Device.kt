 ...
 *  - SOIL_SENSOR 土壤墒情检测器（采集土壤湿度/温度，不可操作）
 */
// 定义一个枚举类，表示所有可能的设备类型
enum class DeviceType {
    VALVE,       // 电动阀（可操作）
    SENSOR,      // 温湿度传感器（不可操作）
    SOIL_SENSOR  // 土壤墒情检测器（不可操作，看墒情）
}
...
data class Device(
    ...
    val soilMoisture: Double = 0.0,   // 土壤体积含水量 %（墒情检测器）
    val soilTemperature: Double = 0.0 // 土壤温度 ℃（墒情检测器）
)
...
/**
    ...
    val soilM = latest("soilMoisture")?.toDoubleOrNull()
    val soilT = latest("soilTemperature")?.toDoubleOrNull()
    ...
    return copy(
        ...
        faultStatus = fault,
        soilMoisture = soilM ?: soilMoisture,
        soilTemperature = soilT ?: soilTemperature,
        lastReportAt = ts
    )
}
