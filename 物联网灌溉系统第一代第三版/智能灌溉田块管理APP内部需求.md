...
| **微服务端** | 自定义定时任务调度服务，负责任务冲突检测、定时扫描、RPC 触发 |
...
┌──────────────────┐     MQTT上行      ┌──────────────────┐
│                   │ ──────────────▶  │                  │
│   温湿度计         │   topic: soilstatus│                  │
│   (不可操作)       │                  │                  │
│                   │                  │                  │
├──────────────────┤                  │   ThingsBoard    │
│                   │     MQTT上行      │   服务端          │
│   电动阀           │ ──────────────▶  │   (v4.0.1)      │
│   (内嵌流量计)     │   topic: valveopen│                  │
│                   │ /valveclose      │                  │
│                   │ ◀────────────── │                  │
│                   │   MQTT下行      / │                  │
│                   │   topic控制指令/   │                  │
...
|----------|-----------|----------|------|
| 温湿度计 | `soilstatus` | 每 10分钟 | 温度、湿度 |
| 电动阀（未工作） | valveoclose | 每 30分钟 | 电量、故障、状态 |
| 电动阀（工作中） | `valveopen` | 每 10 秒 | 流量、水压、电量 |
...
```
温湿度计：
  └─ 上行 ── topic: soilstatus ── 每10分钟 ── 温度/湿度/时间/设备ID
  └─ 下行 ── 无（不可操作设备）

电动阀（非工作状态）：
  └─ 上行 ── topic: valveoclose ── 每30分钟 ── 开关状态/设备ID/故障状态/电量
  └─ 下行 ── topic: v1/devices/me/rpc/request/+ ── 远程控制

电动阀（工作状态）：
  └─ 上行 ── topic: valveopen ── 每10秒 ── 设备ID/瞬时流量/累计用水量/水压/电量
  └─ 下行 ── topic: v1/devices/me/rpc/request/+ ── 远程控制
```
...
\1. 用户未登录时只能访问登录界面，登录直接发api给ThingsBoard服务端验证登录

农田界面

\2. 农田界面可以查看所有农田信息，发api给ThingsBoard查询信息(农田的设备数量，农田名称，农田ID），点击农田会返回该农田下所有的设备，以及他们的信息（温度湿度计返回：设备id,温度，湿度，设备安装的农田，以及记录的时间;电动阀返回：当前状态，电量）

\3. 农田界面可以查看所有农田信息，发api给ThingsBoard查询信息(农田的设备数量，农田名称，农田ID），点击农田会返回该农田下所有的设备列表。

\4. 农田界面单击某一个可操作设备可以选择对其操作开启或者关闭，发api给ThingsBoard操作{设备id,操作开/关(布尔类型）}，可以添加任务发给微服务端{设备id,开始时间（默认立刻），结束时间（间隔时间以1min起步）}，微服务端检验设备id是否有冲突，无冲突则返回添加成功，有冲突不让添加，返回失败，如果添加成功了，微服务端在任务表数据库里加上一条新数据（id,开始时间，结束时间），如果数据库不为空，每隔10秒取出一条数据来建立任务，到了时间自动去执行开启/关闭，执行完微服务端自动去数据库删除任务。

\5. 如果多选设备，发api给ThingsBoard操作{设备id,操作开/关(布尔类型）}，添加定时任务可以发{list[设备ID],开始时间（默认立刻），结束时间},微服务端接收到然后拆解id列表，按顺序一个个（id,开始时间，结束时间）放数据库，每次放任务的时候检查设备id是否有冲突，无冲突则返回添加成功，有冲突则所有的任务不让添加，返回失败，有设备任务冲突，如果添加成功了，微服务端在任务表数据库里加上一条条新数据（id,开始时间，结束时间），批量添加任务，用id拆分成一条条数据，在任务管理的时候也可以方便具体设备的删除

\6. 任务管理，返回所有任务表里剩下的数据，删除没开始的任务数据，可以直接删，如果删除已经开始的任务，就直接给设备发暂停
